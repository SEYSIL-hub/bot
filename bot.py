...
def back_to_chapter_list_callback(call):
    ...
    # 1. Удаляем все сообщения с контентом главы (кроме сообщения с кнопкой "назад")
    for msg_id in content_message_ids:
        try:
            # Пропускаем удаление сообщения, к которому прикреплена текущая кнопка "Назад"
...
def cancel_handler_callback(call):
    # Универсальный обработчик отмены через inline-кнопку
    bot.answer_callback_query(call.id)
    chat_id = call.message.chat.id
    clear_state(chat_id)
    if chat_id in ADMIN_IDS:
        bot.edit_message_text("↩️ Действие отменено. Возврат в админ-панель:", chat_id, call.message.message_id, reply_markup=get_admin_menu_keyboard())
    else:
        # Для обычных пользователей просто возвращаем в меню
        back_to_user_menu_callback(call)
